import json
import time
import os  # Secret key read korar jonno
from playwright.sync_api import sync_playwright

# --- SECRET URL SETUP ---
# GitHub Secret theke link nibe. Jodi local e chalano hoy tobe error dibe na, default link nibe.
BASE_URL = os.environ.get("FAWA_URL")
if not BASE_URL:
    BASE_URL = "http://www.fawanews.sc/" # Backup jodi secret na pay
...
def run():
    with sync_playwright() as p:
        ...
        # --- PART 1: ACCESS SITE ---
        print(f"[-] Accessing Site...") # URL print korchi na security er jonno
        site_loaded = False
        ...
        for link in all_links:
            try:
                # Parent Text Logic (Jate Title + Team Name duto e ase)
                ...
                if is_match:
                    ...
                    if len(lines) >= 2:
                        rivels_text = lines[0] # Top Line
                        title_text = lines[1]  # Bottom Line
                    else:
                        rivels_text = lines[0]
        ...
        for index, match in enumerate(unique_matches):
            ...
            try:
                ...
                if m3u8_found:
                    ...
                    # Secret URL referer hisebe add kora holo
                    formatted_link = f"{m3u8_found}|referer={BASE_URL}"
